...
T = Expression(('0.0' ,' near(x[1],width) && x[0] >= 0.9*length && x[0] <= length ? - traction : 0.0 ' , '0.0'), degree=1, width=width, length=length, traction=traction)
...
xdmffile_disp = XDMFFile('cant_copper_deflection.xdmf')
xdmffile_stress = XDMFFile('cant_copper_stress.xdmf')
xdmffile_vonmises = XDMFFile('cant_copper_vonMises.xdmf')
